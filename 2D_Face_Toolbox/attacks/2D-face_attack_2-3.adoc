...
=== Attack type
Replay video attack, High quality

== Overview
The evaluator records and replays target user's face on a high resolution screen and present it to the TOE.
...
== Recipe
The evaluator shall record a face image and replay it on a high resolution screen. 
...
+
The evaluator can change, for example, ISO number of camera to change quality of face images. Such change may affect the texture of artefacts that may lead to false acceptance. 
...
== Presentation
The evaluator shall present a high resolution screen by tripod for 10 seconds as described in <<2DFaceOverview>>.
