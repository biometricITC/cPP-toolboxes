...
== Guidance on Test Report
As the testing described in the individual toolboxes encompasses the creation and presentation of a large number of artefacts, the report should provide sufficient information to ensure how the artefacts were created, presented and as applicable, stored (or retrieved from storage). 

Broadly, pictures or video (visual) evidence should be used on a per-artefact type basis, such that each type is shown clearly once, and the remainder of artefacts and usage would be recorded as expected. 

The following categories should have some sort of visual evidence:

* Creation of an artefact type
* Use of an artefact type
* If applicable, storage of an artefact type after use
* If applicable, retrieval of an artefact type from storage for use

From a planning standpoint, the easiest way to handle this would be to visually record one artefact from retrieval/production to disposal/storage (depending on the type).
