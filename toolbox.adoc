...
However, the BIO-iTC will explore the possibility of developing a PAD evaluation method at any PAD levels that can be mutually recognized under the CCRA, for example, using multi-assurance evaluation concept that enables the evaluation to conform to multi-assurance security target.
...
ISO/IEC 19989 “Information security — Criteria and methodology for security evaluation of biometric systems” provides such additional information. However, this standard is not specific to the mobile devices and all evaluation activities may not be applied to the mobile biometrics.
