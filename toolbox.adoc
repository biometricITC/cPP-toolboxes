...
As evaluations advance to higher AVA_VAN levels, testing becomes intentionally more open-ended. Rather than following a rigid script, evaluators are afforded greater latitude and "wiggle room" to apply their expertise, explore subtle systemic weaknesses, and iteratively refine attack presentations.

==== Defining "The Box": Scope Boundaries vs. Methodological Freedom
To balance tester autonomy with standard compliance, evaluations at higher levels are governed by well-defined operational boundaries—often referred to as "the box." As the assurance level rises, the perimeter of the box is more strictly defined to guarantee that all testing strictly complies with the targeted attack potential rating and stays within the intended Common Criteria scope.
...
Higher PAD levels remain cumulative supersets in terms of threat coverage. An evaluation at a higher level continues to test the vulnerability vectors and artefact categories introduced at lower levels; however, the depth and rigor of each test change significantly.
...
| *Hybrid / Guided:* Core scripts supplemented with evaluator-driven exploration. 
| *Exploratory:* High evaluator flexibility and expertise-driven probing.
